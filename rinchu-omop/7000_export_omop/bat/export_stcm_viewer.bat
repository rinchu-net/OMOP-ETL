...
CALL %SETPATH%\ConfirmExecution.bat "source_to_concept_map確認用CSV抽出" "%ENVPATH%"
if errorlevel 1 exit /b 0

REM Set target file name
SET TARGET=export_stcm_viewer
...
set VOCAB_LIST_SQL=export_stcm_viewer_vocabulary_list_run.sql
set VOCAB_LIST_FILE=export_stcm_viewer_vocabulary_list.tmp

REM Replace schema name in SQL file
powershell -Command "(Get-Content 'export_stcm_viewer_vocabulary_list.sql') -replace ':working_schema', '%SCHEMA%' | Set-Content '%VOCAB_LIST_SQL%' -Encoding UTF8"
...
for /f "usebackq delims=" %%V in ("%VOCAB_LIST_FILE%") do (
    ...
    powershell -Command "$content = Get-Content 'export_stcm_viewer.sql' -Raw; $content = $content -replace ':working_schema', '%SCHEMA%'; $content = $content -replace ':source_vocabulary_id', '%%V'; $content = $content -replace ':output_file', 'viewer_%%V.csv'; Set-Content 'export_stcm_viewer_run.sql' -Value $content -Encoding UTF8" >> %LOGFILE% 2>&1
    
    REM Execute export
    psql %PARAM% -f "export_stcm_viewer_run.sql" >> %LOGFILE% 2>&1
    set RESULT=%ERRORLEVEL%

    REM Convert to UTF-8 with BOM
    powershell -Command "$content = Get-Content '..\stcm\viewer_%%V.csv' -Raw -Encoding UTF8; $utf8BOM = New-Object System.Text.UTF8Encoding $true; [System.IO.File]::WriteAllText('..\stcm\viewer_%%V.csv', $content, $utf8BOM)" >> %LOGFILE% 2>&1
...
del "export_stcm_viewer_vocabulary_list_run.sql" 2>nul
del "export_stcm_viewer_run.sql" 2>nul

REM Show result
CALL %SETPATH%\ShowResult.bat %RESULT% "source_to_concept_map エクスポート" "%LOGFILE%"
