...
----------------------------------------------
-- Achilles Vocabulary統計情報生成
-- WebAPIからDDL生成
-- http://localhost:8080/WebAPI/ddl/achilles?dialect=postgresql&schema=atlas&vocabSchema=omop
----------------------------------------------
...
/************************************************/
DROP TABLE IF EXISTS :atlasresult_schema.achilles_result_concept_count;

CREATE TABLE :atlasresult_schema.achilles_result_concept_count
(
  concept_id                int,
  record_count              bigint,
...
WITH counts  AS (
  ...
  FROM :atlasresult_schema.achilles_results
...
WITH concepts  AS (
  ...
  from (
    ...
    from tmp_counts c
    join :production_schema.concept_ancestor ca on cast(ca.descendant_concept_id as varchar(50)) = c.concept_id
  ) c
  left join :production_schema.concept_ancestor ca on c.concept_id = cast(ca.ancestor_concept_id as varchar(50))
)
 SELECT
ancestor_id,
  descendant_id
...
INSERT INTO :atlasresult_schema.achilles_result_concept_count (concept_id, record_count, descendant_record_count, person_count, descendant_person_count)
SELECT DISTINCT
