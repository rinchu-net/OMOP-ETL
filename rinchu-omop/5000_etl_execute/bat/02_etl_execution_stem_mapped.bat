...
CALL %SETPATH%\ConfirmExecution.bat "中間テーブルマッピング付与" "%ENVPATH%"
if errorlevel 1 exit /b 0

REM Set target file name
SET TARGET=02_etl_execution_stem_mapped
...
REM Display results
CALL %SETPATH%\ShowResult.bat %RESULT% "中間テーブルマッピング付与" "%LOGFILE%"
