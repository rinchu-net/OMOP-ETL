...
for(targetInd in seq_len(length(targetsC$cohortDefinitionId))){
cat(paste0('\n\n### ',targetsC$cohortFriendlyName[targetInd], ' \n\n'))

  cat('\n\n:::: {.panel-tabset .nav-pills}')
  for(outcomeInd in seq_len(length(outcomes$cohortDefinitionId))){
...
sexPlot <- NULL
        if(nrow(sexData)>0){
          sexData$caseAverage <- as.double(sexData$caseAverage)
          sexData$nonCaseAverage <- as.double(sexData$nonCaseAverage)
...
if(nrow(ageData)>0){
  ...
  ageData$caseAverage <- as.double(ageData$caseAverage)
  ageData$nonCaseAverage <- as.double(ageData$nonCaseAverage)
