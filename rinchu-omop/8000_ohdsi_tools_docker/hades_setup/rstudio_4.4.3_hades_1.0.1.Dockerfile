...
RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/DatabaseConnector', ref='v7.1.0'); \
...
RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/Cyclops', ref='v3.6.0'); \
             stopifnot(requireNamespace('Cyclops', quietly=TRUE))"

RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('duckdb/duckdb-r', ref='v1.3.0'); \
             stopifnot(packageVersion('duckdb') >= '1.3.0')"

RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/Andromeda', ref='v1.2.0'); \
             stopifnot(packageVersion('Andromeda') >= '1.2.0')"

RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/FeatureExtraction', ref='v3.13.0'); \
...
RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/DataQualityDashboard', ref='v2.8.7'); \
...
RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/MethodEvaluation', ref='v2.4.0'); \
...
RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/CohortGenerator', ref='v1.1.0'); \
...
RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/PhenotypeLibrary', ref='v3.36.0'); \
             stopifnot(requireNamespace('PhenotypeLibrary', quietly=TRUE))"

RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/PheValuator', ref='v2.2.16'); \
...
RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/Keeper', ref='v0.2.1'); \
...
RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/CohortMethod', ref='v6.0.1'); \
             stopifnot(requireNamespace('CohortMethod', quietly=TRUE))"

RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/SelfControlledCaseSeries', ref='v6.1.1'); \
             stopifnot(requireNamespace('SelfControlledCaseSeries', quietly=TRUE))"

RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/SelfControlledCohort', ref='v1.6.0'); \
...
RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/Characterization', ref='v3.0.0'); \
...
RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/OhdsiReportGenerator', ref='v2.1.0'); \
             stopifnot(requireNamespace('OhdsiReportGenerator', quietly=TRUE))"

RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/OhdsiShinyModules', ref='v3.5.0'); \
             stopifnot(requireNamespace('OhdsiShinyModules', quietly=TRUE))"

RUN --mount=type=secret,id=ghpat \
    R -q -e "Sys.setenv(GITHUB_PAT = trimws(paste(readLines('/run/secrets/ghpat', warn=FALSE), collapse=''))); \
             options(repos = c(CRAN='https://packagemanager.posit.co/cran/__linux__/noble/latest')); \
             remotes::install_github('OHDSI/OhdsiShinyAppBuilder', ref='edb96fb1b97a970a1fa178e7004e44f3e0aa2a15'); \
