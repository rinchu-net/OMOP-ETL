...
============================================================
  ...
  3. cte_02_icd10
     - MEDIS病名マスタ病名交換用コードとICD10のconcept_idの対応表

  4. cte_03_standard
     - MEDIS病名マスタ病名交換用コードとStandard concept_idの対応表
  ...
  vocabulary_id:
    MEDIS_DIAGEXC_S : 病名交換用コード to Standard Concept(ICD10からMaps toで関連付けられたstandard concept_id)
    MEDIS_DIAGEXC_N : 病名交換用コード to Non-Standard Concept(ICD10)
    MEDIS_DIAGEXC_V : 病名交換用コード to value用Standard Concept(ICD10からMaps to valueで関連付けられたstandard concept_id)
============================================================
*/
CREATE OR REPLACE VIEW :working_schema.v_source_to_concept_map_medis_diagexc AS
WITH
...
cte_01_byomeimst AS (
    ...
    FROM (
        SELECT 
            ROW_NUMBER() OVER (PARTITION BY by.exchangecd ORDER BY by.kanrino) AS rn,
            by.*,
            CASE
...
-- MEDIS病名マスタの病名交換用コードとICD10のconcept_idの対応表を生成
...
cte_02_icd10 AS (
    WITH combined_icd AS (
        ...
        WHERE by.icd10_2 IS NOT NULL AND icd10_2 != ''
    )
    SELECT
        c.*,
        c.exchangecd AS byomei_cd,
    ...
    LEFT JOIN :working_schema.concept_f con_partial ON con_partial.vocabulary_id = 'ICD10' AND (con_partial.concept_code = c.icd_code || '0' OR con_partial.concept_code || '0' = c.icd_code)
...
-- MEDIS病名マスタの病名交換用コードとstandard concept_idの対応表を生成
cte_03_standard AS (
    SELECT
        icd.*,
        r.relationship_id,
...
-- MEDIS病名マスタの病名交換用コードとicd10 / standard concept_idの対応表を統合
cte04_icd_standard_union AS (
/* 病名交換用コード to icd10(non-standard)マッピング は使用しないため出力しない
    SELECT
        'N' AS standard_flg,
...
cte_final_prep AS(
	SELECT
        ...
        source_concept_id,
        CASE WHEN relationship_id = 'Maps to value' AND target_domain_id = 'Meas Value' THEN 'MEDIS_DIAGEXC_V'
        ELSE
            CASE 
                WHEN standard_flg = 'Y' THEN 'MEDIS_DIAGEXC_S'
                ELSE 'MEDIS_DIAGEXC_N'
            END
...
DELETE FROM :working_schema.source_to_concept_map_f WHERE source_vocabulary_id LIKE 'MEDIS_DIAGEXC%';
INSERT INTO :working_schema.source_to_concept_map_f SELECT * FROM :working_schema.v_source_to_concept_map_medis_diagexc;
SELECT * FROM :working_schema.source_to_concept_map_f WHERE source_vocabulary_id LIKE 'MEDIS_DIAGEXC%';
SELECT DISTINCT source_vocabulary_id, count(1) FROM :working_schema.source_to_concept_map_f WHERE source_vocabulary_id LIKE 'MEDIS_DIAGEXC%' GROUP BY source_vocabulary_id;
*/
...
FROM  :working_schema.v_source_to_concept_map_medis_diagexc
LEFT JOIN :working_schema.concept_f c ON target_concept_id = c.concept_id;cd..
*/
